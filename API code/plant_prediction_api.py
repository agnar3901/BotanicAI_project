...
# Your custom layer definition (replace with your actual class)
# class HubKerasLayer(hub.KerasLayer):
#     pass
# tf.keras.utils.get_custom_objects().update({'KerasLayer': HubKerasLayer})
...
#     model = tf.keras.models.load_model(model_path, custom_objects={'KerasLayer': HubKerasLayer})

#     # Update the custom_objects dictionary
#     model._layers = [layer for layer in model._layers if isinstance(layer, HubKerasLayer)]
#     model.custom_objects = {'KerasLayer': HubKerasLayer}

#     return model

def load_plant_detection_model():

  #model_path = 'batanikaya_model_prototype.hdf5'
  try:
    model = tf.keras.models.load_model("batanikaya_model_prototype.hdf5", custom_objects=custom_objects)
...
print(plant_names_array)

@app.post("/predict") 
async def predict(image: UploadFile = File(...)): #getting the data from the app post call.
  ...
  try:
    ...
    if model is None:
      raise HTTPException(status_code=500, detail="Failed to load model")  # Raise internal server error

    prediction = model.predict(np.expand_dims(preprocessed_image, axis=0))
    plant_class_index = np.argmax(prediction)
    prediction_str = str(prediction)
    plant_class_index_str = str(plant_class_index)
    plant_name = plant_names_array[plant_class_index]

    # Return the prediction results in JSON format "probability": prediction[0][0]
    return {"plant_name": plant_name}
  except Exception as e: 
    print(f"Error during prediction: {e}")
    raise HTTPException(status_code=400, detail="An error occurred during prediction")  # Raise bad request error
...
if __name__ == "__main__":
    uvicorn.run(app, host="192.168.254.199",port=8000)  # Change port here
